...
public class ThirdActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private ShopCat3Adapter adapter;
    ...
    private void initListView() {
        adapter = new ShopCat3Adapter(groups, childs, this);
        //关键步骤1：设置复选框的接口
        adapter.setCheckInterface(new ShopCat3Adapter.CheckInterface() {
            /**
        ...
        //关键步骤2:设置增删减的接口
        adapter.setModifyCountInterface(new ShopCat3Adapter.ModifyCountInterface() {
            @Override
        ...
        //关键步骤3:监听组列表的编辑状态
        adapter.setGroupEditorListener(new ShopCat3Adapter.GroupEditorListener() {
            @Override
        ...
        //ExpanlistviewUtility.setListViewHeightBasedOnChildren(mListView,0);
        mListView.addFooterView(getFootView());
        mListView.addFooterView(getFootRecommendView());
    ...
    private View getFootRecommendView() {
        ...
        for(int a=0 ; a<20 ; a++){
            ShopBean bean = new ShopBean(R.drawable.bg_kites_min,"这个是猜你喜欢的标题"+a);
            moreShopList.add(bean);
        }
        moreAdapter.addAll(moreShopList);
        moreAdapter.notifyDataSetChanged();

        // 设置该方法，可以解决滑动冲突和卡顿。但是无法加载所有更多数据和上拉加载更多
        //recyclerView.setNestedScrollingEnabled(false);

        moreAdapter.setMore(R.layout.view_recycle_more, new RecyclerArrayAdapter.OnMoreListener() {
            @Override
